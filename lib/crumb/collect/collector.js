...
class Collector {
    ...
    fillTemplate(jobs, headers, row, rowId)
    {
        let steps = [];
        ...
        for( var step of jobs.job )
        {
            ...
            let regex = new RegExp(`[$]row`, 'g');
            step = step.replace(regex, rowId );
    ...
    async spread(data, rowNumber, headers, jobs, verbose) {
        ...
        let steps = this.fillTemplate(jobs, headers, data, rowNumber);
        await node.run(steps, rowNumber, verbose);
    }
